...
namespace TASBoardConsole.MovieReaders
{
    public class LibTASReader : IMovieReader
    {
        ...
        private sealed class LibTASEnum : IEnumerator<InputFrame>
        {
            ...
            public bool MoveNext()
            {
                ...
                //Super precise framerates for T-FT correction cause accumulation problems
                //As such, scale them down to a more reasonable approximation
                if (framerateNum >= 1000 && framerateDen >= 1000)
                {
                    var oldNum = framerateNum;
                    framerateNum = 1000;
                    framerateDen = (int)Math.Round(1000*(double)framerateDen/oldNum);
                }

                // Reciprocal of framerate is timedelta
                var delta = new Fraction(framerateDen, framerateNum);
                var nextTime = _time + delta;
                currentFrame = new(keys, delta, new TimeInterval((double)_time, (double)nextTime));
                _time = nextTime;
            ...
            static Dictionary<string, string> GetKeySymToNameDict()
            {
                ...
                var mappingsStream = File.OpenRead("InputMappings/LibTAS.txt");
